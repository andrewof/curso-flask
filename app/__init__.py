    ...
    if current_user.is_authenticated:
        if current_user.tipousuario.id == 1:
            try:
                ...
                return render_template('errores/errores.html', mensaje=format(ex))
        else:
            try:
...
# Iniciar el servidor en modo debug y que autoactualice
if app.config['DEBUG']:
    app.config['TEMPLATES_AUTO_RELOAD'] = True


# Controlar el error 404 - página no econtrada
app.register_error_handler(404, pagina_no_encontrada)
# Contralar el error 401 - pa´gina no autorizada
